...
export const Settings: React.FC = () => {
  ...
  const handleExportData = async () => {
    ...
    const exportData: Record<string, unknown> = { exported_at: new Date().toISOString() };
  ...
  const handleDeleteAccount = async (e: React.FormEvent) => {
    ...
    if (!res.ok) {
      setDeleteMsg(`Error: ${json.error || 'Account deletion failed. Please try again.'}`);
      setDeleteLoading(false);
      return;
    }
    ...
    <div className="settings-container">
      ...
      <section className="settings-section">
        ...
        <p className="settings-description">
          Download all your account data as a JSON file. This includes all policies, cookie settings, and visitor consent logs (GDPR Article 20 — right to data portability).
        </p>
        <button onClick={handleExportData} className="settings-btn" disabled={exportLoading || !userId}>
      ...
      <section className="settings-section settings-danger-zone">
        <h2 className="settings-section-title settings-danger-title">Delete Account</h2>
        <p className="settings-description">
