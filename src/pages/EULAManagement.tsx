...
export const EULAManagement: React.FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this EULA? This cannot be undone.')) return;
    setDeletingId(id);
    await supabase.from('eula').delete().eq('id', id);
      ...
      <div className="pm-header">
        <div>
          ...
          <p className="pm-description">
            Generate and manage your End User License Agreements using our step-by-step wizard.
          </p>
        </div>
      </div>
        ...
        {isLoading ? (
                  ...
                  <td>
                    <div className="pm-actions">
                      <button
                        ...
                        title="Preview"
                        onClick={() => navigate(`/eula/${policy.id}/preview`)}
                      >
                        <Eye size={15} />
                      </button>
                      <button
                        className="pm-btn-icon"
                        title="Edit"
                        onClick={() => navigate(`/eula/${policy.id}/edit`)}
                      >
                        <Pencil size={15} />
                      </button>
                      <button
          ...
          <div className="pm-empty">
            ...
            <p className="pm-empty-sub">
              Use our step-by-step wizard to create your first End User License Agreement.
            </p>
            <button
              className="btn-create"
              onClick={() => navigate('/eula/new')}
            >
              <Plus size={16} /> Create New EULA
            </button>
          </div>
        )}
      </div>
