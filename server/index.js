...
const CATEGORY_TEMPLATES = [
  ...
  },
  {
    id: 'functional',
    name: 'Functional',
    description: 'These cookies are used to enhance the performance and functionality of our websites but are non-essential to their use. However, without these cookies, certain functionality (like videos) may become unavailable.',
    providers: []
  },
  {
    id: 'analytics',
...
app.post('/api/scan', async (req, res) => {
  ...
  try {
    ...
    for (const cookie of cookies) {
      ...
      let descriptionStr = `Generated by ${providerName}. `;
      if (cookie.isStorage) {
        descriptionStr += `Stored in ${cookie.storageType}.`;
      } else {
        descriptionStr += `Expires in ${cookie.expires > 0 ? Math.round((cookie.session ? 0 : cookie.expires - Date.now() / 1000) / 86400) + ' days' : 'Session'}`;
      }
...
app.get('/api/banner/:id', async (req, res) => {
  ...
  const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im95Zmp3bmV5YmhseWJmbWJnaWxuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQwMzk3NTYsImV4cCI6MjA4OTYxNTc1Nn0.mPTYIf5q3OnWK88elyPqI_tfX4EJ4h91SmEuRN3AK44";
  
  try {
    const response = await fetch(`${SUPABASE_URL}/rest/v1/user_cookie_settings?user_id=eq.${id}&select=banner_config,scanned_data`, {
      headers: {
